...
export const athleteAchievements = onRequest(
  { region: REGION, secrets: [DOMTEL_TOKEN, APP_API_KEY], cors: true },
  async (req, res) => {
    // 1) Preflight (przeglądarka może pytać bez nagłówków)
    if (req.method === "OPTIONS") {
      res.status(204).end();
      return;
    }

    // 2) Proste zabezpieczenie: tylko nasza apka zna klucz
    ...
    try {
      ...
      const cacheKey = `osiagniecia_${athleteId}`;
      ...
      const items = await domtelFetch("osiagniecia.php", token, {
      ...
      console.error("[athleteAchievements] error", e);
